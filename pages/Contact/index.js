...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !description || !time   ) {
      ...
      try {
          await fetch(`https://api.telegram.org/bot${telegram_bot_id}/sendMessage`, settings);
          toast({
  ...
  return (
    <form action="" onSubmit={handleSubmit} className='sm:w-[600px] mx-auto  mt-24 flex flex-col justify-center items-center h-[600px] '>
      <div className='sm:w-full flex flex-col gap-10 mt-5'>
        <Input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder="Your Name" className='p-6 rounded-sm ' />
        <Input onChange={(e) => setDescription(e.target.value)} value={description} type="email" placeholder="Email" className='p-6 rounded-sm ' />
        <Textarea onChange={(e) => setTime(e.target.value)} className='rounded-sm h-[200px] ' value={time} placeholder="Type your message here." />
        <Button type="submit" className='bg-blue-500 font-bold py-2 px-4 rounded text-white hover:-mt-5 flex gap-2 hover:bg-red-600 p-6' style={{ transition: "1s all" }}>
          <div><p>Send</p></div>
